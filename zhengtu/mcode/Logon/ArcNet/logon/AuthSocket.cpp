...
void AuthSocket::HandleChallenge()
{
	...
	char *recv_str = (char *)readBuffer.GetBufferStart();
	...
	if(ipb != BAN_STATUS_NOT_BANNED)//如果不等，则该IP被禁，不能登录
	...
	// Check that the account isn't banned.检查账号是否被禁
...
void AuthSocket::OnRead()
{
	...
	FS_PACKET* pPacket = new FS_PACKET;
	readBuffer.Read(pPacket,sizeof(FS_PACKET));
	last_recv = UNIXTIME;

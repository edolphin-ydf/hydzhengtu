				 ...
				 m_dwMySendCount(0)
{
	...
	{
		// [ranqd] 绑定IO到完成端口上
		zIocp::getInstance().BindIocpPort( (HANDLE)sock, this );
	}
...
// [ranqd] 投递IO读请求
void zSocket::ReadByte( DWORD size )
{
	BOOL    bresult;
	int     err;
	DWORD   numRead;
	...
	if (err == 64)//连接已关闭
	{
		zIocp::getInstance().PostStatus(this,&m_ovIn);//发送读取失败的完成信息
		return;
	}
	if( err != 997 )
	{
		Zebra::logger->debug( "接收数据时重叠I/O操作发生错误： %s",GetErrMsg(err) );
	}
}
// [ranqd] 通过Iocp发送数据
int zSocket::SendData( DWORD dwNum = 0 )
{
	BOOL    bresult;
	int     err;
	DWORD   numRead;
	...
	if( err == 64 )//连接已关闭
	{
		zIocp::getInstance().PostStatus(this,&m_ovOut);//发送写数据失败的完成信息
		return -1;
	}

	if( err != 997 )
	{
		Zebra::logger->debug( "发送数据时重叠I/O操作发生错误： %s",GetErrMsg(err) );
		return -1;
	}
...
void zSocket::RecvData( DWORD dwNum )
{
	...
	if( m_RecvBuffer.rd_size() == PACKHEADSIZE )
	{
		...
		{
			Zebra::logger->debug("Iocp收到错误的包头来自： %u.%u.%u.%u : %u", addr.sin_addr.S_un.S_un_b.s_b1,addr.sin_addr.S_un.S_un_b.s_b2,addr.sin_addr.S_un.S_un_b.s_b3,addr.sin_addr.S_un.S_un_b.s_b4, addr.sin_port / 0xFF + addr.sin_port % 256 * 256);
			Zebra::logger->debug16("错误数据：", m_RecvBuffer.rd_buf(), m_RecvBuffer.rd_size() );
			DisConnet();
			goto ret;
		}
		else
...
int zSocket::WaitRecv( bool bWait, int timeout )
{
	if( bWait )
	{
...
int zSocket::WaitSend( bool bWait, int timeout )
{
	if( bWait )
	{
		DWORD EnterTime = GetTickCount();
...
{
	//Zebra::logger->debug("zSocket::recvToCmd");
...
int  zSocket::Send(const SOCKET sock, const void* pBuffer, const int nLen,int flags)
{
...
{
	//fprintf(stderr,"zSocket::sendRawData\n");
...
{
	//Zebra::logger->debug("zSocket::sendCmd");
...
{
	//Zebra::logger->debug("zSocket::force_sync");
	if (_enc_queue.rd_ready())
	{
		mutex.lock();
...
{
	//Zebra::logger->debug("zSocket::recvToCmd_NoPoll");
	//够一个完整的纪录,不需要向套接口接收数据
	do { 
		if (_rcv_raw_size >= packetMinSize()/* && _rcv_queue.rd_size() >= packetMinSize()*/) 
		{ 
...
{
	//Zebra::logger->debug("zSocket::setNonblock");
	int nodelay = 1;
...
DWORD zSocket::packetUnpack(BYTE *in,const DWORD nPacketLen,BYTE *out)
{
	...
	if (PACKET_ZIP == ((*(DWORD*)(in + PACKHEADSIZE)) & PACKET_ZIP))
	{
		...
		if (Z_OK != retcode)
		{
			Zebra::logger->fatal("zSocket::packetUnpack %d.",retcode);
		}
		//返回得到的字节数

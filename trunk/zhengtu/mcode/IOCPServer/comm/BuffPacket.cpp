...
bool CBuffPacket::AddBuffPacket(uint32 u4Size)
{
	...
	try
	{
		...
		szNewData = new char[m_u4PacketLen];
		...
		{
			//移动内存
			::memmove(szNewData, m_szData, u4OldPacketLen);
...
bool CBuffPacket::RollBack(uint32 u4Len)
{
	...
	else
	{
		...
		::memmove(m_szData, m_szData + u4Len, u4NewLen);
	...
	if(m_u4WritePtr + (uint32)sizeof(str.u1Len) + str.u1Len >= GetPacketSize())
	{
		...
		else
		{
			...
			*this << str.u1Len;
			...
			//写入文本内容
			::memcpy(WritePtr(), str.text, (int)str.u1Len);
			WritePtr(str.u1Len);
			return *this;
		}
	}
	...
	if(m_u4WritePtr + (uint32)sizeof(str.u2Len) + str.u2Len >= GetPacketSize())
	{
		...
		else
		{
			...
			*this << str.u2Len;
			...
			//写入文本内容
			::memcpy(WritePtr(), str.text, (int)str.u2Len);
			WritePtr(str.u2Len);
			return *this;
		}
	}
	...
	if(m_u4WritePtr + (uint32)sizeof(str.u4Len) + str.u4Len >= GetPacketSize())
	{
		...
		else
		{
			...
			*this << str.u4Len;
			...
			//写入文本内容
			::memcpy(WritePtr(), str.text, (int)str.u4Len);
			WritePtr(str.u4Len);
			return *this;
		}
	}
